...
- `SECRET_BUNDLES` / `secrets(*groups)`: Named bundles of GitHub Actions secret references (`:ssh`, `:github_token`, `:aws`) merged into a step's `with:`, so a step names the bundle it needs instead of repeating the `${{secrets.*}}` literals
