...
- Rescues `StandardError` (covering `GHB::ConfigError`, `GHB::GitHubAPIError` and `GHB::RepositorySettingsError`), prints `Error: <message>` to stderr and exits 1
- Walks the exception's `cause` chain, printing each link as an indented `caused by: <class>: <message>` line, so a wrapped failure keeps its origin
- Prints the backtrace only when `ENV['DEBUG']` is set
...
- `RepositorySettingsError`: Custom exception class raised by `GHB::RepositoryConfigurator` when repository settings could not be applied (branch protection check mismatch, or a force-push allowlist that could not be cleared)
...
- `SECRET_BUNDLES` / `secrets(*groups)`: Named bundles of GitHub Actions secret references (`:ssh`, `:github_token`, `:aws`) merged into a step's `with:`, so a step names the bundle it needs instead of repeating the `${{secrets.*}}` literals. `:aws` passes `aws-role-to-assume: ${{vars.AWS_ROLE_TO_ASSUME}}` for GitHub OIDC and expands the access-key inputs to empty while that repository or organization variable is set (configure-aws-credentials given both would sign the role assumption with the keys), so a repository keeps authenticating with `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` until the variable exists. `LEGACY_AWS_CREDENTIALS` holds the key-only references the bundle generated before
- `RETIRED_SECRETS`: Secret references that no longer resolve, mapped to their live replacement (`${{secrets.GH_PAT}}` → `${{github.token}}`). Applied by `GHB::Workflow#write`, the one pass every generated file goes through, because `Step#copy_properties` would otherwise carry a pre-retirement reference forward on every regeneration — and a retired secret expands to empty, failing whatever consumes it. `GH_BOT_PAT` is *not* retired: the generated auto-approve workflow still needs it to resolve
- `RUNNER_FAMILIES` / `runner_image(runs_on)`: Maps a runner family (`ubuntu`, `macos`) to its default image, defaulting to `DEFAULT_UBUNTU_VERSION` when unset and passing an unrecognised value through as an explicit runner image
- `generated_header(source_file)`: Builds the `# AUTO-GENERATED by github-build (<source_file>)` banner prepended to every generated workflow
...
- `execute`: Main entry point that outputs ignored folders (if requested) or delegates to builders and managers in sequence. `detect_default_branch` runs here rather than in `initialize`, after the ignored-folders early return, so `--get_ignored_folders` answers from the config files alone without shelling out to git
...
- `validate_language_dependencies(data, relative_path)`: Validates the `dependencies` entries of `languages.yaml`
- `validate_language_failure_artifacts(data, relative_path)`: Rejects a `failure_artifacts` value in `languages.yaml` that is not a list of entries with a non-empty `marker` string and a non-empty list of `paths` strings
...
- `workflow_write`: Writes the generated workflow to YAML file, right after `Workflow#adopt_aws_role` has run over every generated job
...
- `excluded_paths_pattern(excluded_paths)`: Builds a single regexp from the caller-supplied exclusion paths (submodules, `--excluded_folders`, the linters config file) and `excluded_dirs_from_config`, so each scanned path is tested once. Each entry is escaped, stripped of a leading `./` and trailing `/`, and matched only as whole path segments, so `app` excludes `./app/` but not `./application/`
...
- `file_shebang_matches?(file, pattern)`: Matches a file's **first line only** against a regex, so a file is identified as a script by the shape of its shebang (`#!/bin/sh`, `#!/usr/bin/env bash`, `#!/bin/bash -e`) rather than by a fixed substring appearing anywhere in it. Returns `false` for a missing/unreadable file, an empty file, or a binary first line that is not valid UTF-8
...
- `FORMATS`: Maps each managed config file name (`.eslintrc.json`, `.flake8`, `.bandit`, `.yamllint.yml`, `.pmd.xml`, `.semgrepignore`, `.cfnlintrc`, `.swiftlint.yml`, `.markdownlint-cli2.yaml`, `trivy.yaml`) to its native-syntax body renderer
...
- `build`: Detects languages, checks for database dependencies (MongoDB, MySQL, Redis, OpenSearch), validates versions, and creates test jobs. When one of the language's `failure_artifacts` markers matches at the repo root (`failure_artifact_paths`), adds an `Upload Failure Artifacts` step (`build_failure_artifacts_step`: `if: failure()`, `actions/upload-artifact`, the union of the matched entries' `paths`, artifact name suffixed with `${{strategy.job-index}}`) after the test step, so each leg of a hand-added matrix (e.g. Rails `test` / `test:system`) uploads its own screenshots or reports; its `if:` and `with:` are kept from the previous workflow and its action version is refreshed from `config/actions.yaml`. For Swift projects with Xcode Cloud (`ci_scripts` directory), builds the unit test job so its dependency steps are still collected (CodeDeploy pre-steps, `Podfile.lock` licenses step) and then deletes the job from the workflow
...
- `cache_skip_reason(cache_names)`: Returns the message printed when caching is left off (no cacheable package manager, or two detected)
...
**Purpose:** Manages auto-approve workflow generation for code owners, providing automatic approval of pull requests authored by CODEOWNERS. The workflow only approves — it never merges — which is why it is named "Auto-approve".

**Location:** `lib/ghb/auto_approve_manager.rb`
...
**Purpose:** Removes the dependabot config and the retired cron dependency workflows from the target repository. Generates nothing.

**Location:** `lib/ghb/dependabot_manager.rb`

**Key Components:**

- `save`: Removes `.github/dependabot.yml` if present (CVE alerts are handled by repository settings), then removes `.github/workflows/dependencies.yml` and `.github/workflows/soup.yml` unconditionally. The cron dependency workflow was retired because it wrote `secrets.GH_PAT` into `~/.gitconfig` in cleartext and then ran package-manager updates whose install scripts execute third-party code in the same job; dependency updates now run outside CI. Deleting both files on every run cleans up existing repositories instead of orphaning a workflow nothing maintains
...
- `update`: Detects templates, fetches from API, validates the response body with `GHB::GitignoreRules#validate_template!` before any transform or write (so a rejected body raises `ConfigError` and leaves `.gitignore` untouched), applies modifications, appends a single managed section — one `# BEGIN Managed patterns` / `# END Managed patterns` block whose body holds one blank-line-separated group per tool or credential set, built from `GHB::GitignoreRules#detect_custom_pattern_groups` — and writes `.gitignore`. The groups are flattened here into the pattern list handed to `GHB::GitignoreRules#preserve_custom_entries` for its "skip already-managed lines" comparison
...
- `validate_template!(body)`: Raises `ConfigError` unless the gitignore.io body is at most 1 MB, valid UTF-8 without NUL/control characters (tab, CR and LF allowed), not HTML, framed by the `# Created by https://www.toptal.com/developers/gitignore/api/` header line and `# End of https://www.toptal.com/developers/gitignore/api/` footer line, and every non-blank, non-comment line is a sane pattern: not a bare `!`, at most 1024 characters, no trailing unescaped backslash, and not a non-negated catch-all (`*`, `**`, `*.*`, `**/*` after trimming surrounding slashes) that would ignore the whole repository
...
- `adopt_aws_role`: Moves every step whose `with:` still carries exactly `GHB::LEGACY_AWS_CREDENTIALS` onto the current `:aws` bundle (other inputs, the region included, are kept; customised keys are left alone), because `Step#default_with` would otherwise copy the key-only references forward on every regeneration. Then grants `id-token: write` to each job with an `aws-role-to-assume` step, without which the runner issues no OIDC token: a job with no permissions of its own first inherits the workflow's (or `contents: read`), since a job-level block replaces them entirely. Idempotent
- `write(file, header:)`: Writes workflow to YAML file, rewriting `${GITHUB_*}` references to `${{github.*}}` in YAML values via `rewrite_github_refs` while preserving them in shell `run:` bodies (where they are runner-exported env vars). `${{secrets.GITHUB_TOKEN}}` is deliberately **not** rewritten to `${{secrets.GH_PAT}}`: the former blanket rewrite put a long-lived org-scoped PAT into every step's environment and silently overrode a `GITHUB_TOKEN` a user had set on purpose in a preserved job. The rewrite that does run goes the other way — `rewrite_retired_secrets` replaces every `GHB::RETIRED_SECRETS` reference (`${{secrets.GH_PAT}}`) with `${{github.token}}`, `run:` bodies included, since a retired secret is dead in every position it can appear. A step needing more than the run token names an explicit, still-live secret
- `do_job(id, &block)`: DSL method to define jobs. A block taking one parameter is yielded the `GHB::Job`, so builders call its DSL on an explicit receiver; a zero-arity block is `instance_eval`ed against the job for the older implicit-receiver form
...
**Purpose:** Shared `copy_properties` implementation for the workflow data models, so `Job` and `Step` carry hand-made edits over from a previously generated workflow through one implementation instead of two. Each including class declares its own `COPYABLE_PROPERTIES`.

**Location:** `lib/ghb/workflow/copyable_properties.rb`

**Key Components:**

- `copy_properties(object, properties = self.class::COPYABLE_PROPERTIES)`: Copies each listed property from `object` onto the receiver, returning early on a `nil` source and raising `ArgumentError` when the source does not respond to a requested property
...
- `copy_properties(object, properties)`: Copies properties from another job, defaulting to `COPYABLE_PROPERTIES` (provided by `GHB::CopyableProperties`)
- `do_step(name, options, &block)`: DSL method to define steps. Like `Workflow#do_job`, a one-parameter block is yielded the `GHB::Step` while a zero-arity block is `instance_eval`ed against it
...
- `copy_properties(object, properties)`: Copies properties from another step, defaulting to `COPYABLE_PROPERTIES` (provided by `GHB::CopyableProperties`)
...
All dependencies are managed via Bundler with versions locked in `Gemfile.lock`. `.soup.json` at the repository root is the source of truth for the hand-authored fields below; `docs/soup.md` is generated from it by the `cloud-officer/ci-actions/soup` step in the build workflow and is never edited by hand.
...
3. Skips any linter disabled on the command line — a key listed in `--ignored_linters`, or Semgrep under `--skip_semgrep` — deleting its config files first so a disabled linter leaves nothing behind
4. For each remaining linter, uses pure Ruby `find_files_matching` with regex pattern matching to search for files
5. Excludes specified folders, submodules, config-driven directories, and gitignored paths from search
6. If a `content_match` string is configured, further filters matched files by checking file contents via `file_contains?`. When `content_match_pattern` is also set, only files whose path matches that sub-pattern require the content check; other files pass through unconditionally
7. If a `shebang_match` regex is configured, keeps every file matched by extension as-is and requires the extensionless ones to carry a matching shebang on their first line (via `file_shebang_matches?`), so ShellCheck picks up extensionless shell deliverables without being handed a Ruby or Python script it would fail on with SC1071
8. If matching files remain, enables the linter and resolves each of its configuration files via a priority chain. A linter's `config` may be a single file name or a list (e.g., Trivy ships `["trivy.yaml", ".trivyignore"]`); `configs_for` normalizes it and each entry is resolved independently: cleans up deprecated config files that were renamed (tracked via `RENAMED_CONFIGS` constant, e.g., `.markdownlint.yml` → `.markdownlint-cli2.yaml`), preserves existing project-specific configs (when `preserve_config` is set and a non-symlink file exists, e.g., KTLint's `.editorconfig`), creates symlinks to a scripts submodule `linters/` directory, creates symlinks to a local `linters/` directory (both skipped for `.editorconfig`, which is always written as a real file rather than a symlink, and for a *project-owned* config — see `project_owned_config?`: a merge-managed config the project has deliberately turned from a symlink into a real file carrying the sentinel block, which re-symlinking would silently discard), or falls back to `atomic_copy_config` to safely copy bundled configs with optional transformation (e.g., regenerating the excluded-dirs block via `GHB::LinterIgnoreRenderer` for managed configs, or uncommenting Rails rules in `.rubocop.yml`). For merge-managed configs (`trivy.yaml`), `config_source` points `atomic_copy_config` at the project's own existing file when it already carries the sentinel block, so the managed `scan.skip-dirs` block is regenerated while project-added entries outside it (extra skip-dirs, or a repo-local `scan.skip-files`) are preserved. This is what lets a repo whose shared configs come from a scripts submodule carry an exclusion that must not leak into the other repos sharing that submodule
9. Creates workflow job with appropriate steps for each enabled linter
...
9. For Swift projects with Xcode Cloud (`ci_scripts` directory), builds the unit test job so its dependency steps are still collected (CodeDeploy pre-steps, `Podfile.lock` licenses step), then deletes the job from the workflow
...
- The generator emits no long-lived organization PAT at all: `GHB::DependabotManager#save` deletes the retired cron dependency workflow, which wrote `secrets.GH_PAT` into `~/.gitconfig` before running package-manager updates in the same job, and `GHB::Workflow#write` rewrites every surviving `GHB::RETIRED_SECRETS` reference to `${{github.token}}`, so a workflow generated before the retirement stops carrying the PAT on its next regeneration
...
- `GHB::RepositorySettingsError` raised by `GHB::RepositoryConfigurator` when repository settings could not be applied (required-status-check mismatch, or a force-push allowlist that survived clearing)
